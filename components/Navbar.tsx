...
export default function Navbar() {
  ...
  const handleClick = (e: React.MouseEvent<HTMLLIElement>, index: number, item: string) => {
    ...
    if (window.innerWidth < 640) {
      ...
      if (item === "Game") document.getElementById("game")?.scrollIntoView({ behavior: "smooth" });
      if (item === "About Us") document.getElementById("about")?.scrollIntoView({ behavior: "smooth" });
      if (item === "Career") document.getElementById("careers")?.scrollIntoView({ behavior: "smooth" });
        ...
        <div className="sm:hidden fixed inset-0 z-[90] bg-black/60 backdrop-blur-2xl flex flex-col justify-center items-center text-center font-heading tracking-widest text-sm text-white/70 uppercase">
          <ul className="flex flex-col gap-8 mb-12">
            {navItems.map((item, i) => (
              <li
                key={item}
                className={`cursor-pointer transition-colors ${i === activeIndex ? 'text-white drop-shadow-[0_0_10px_rgba(255,255,255,0.8)] font-bold' : 'hover:text-white/80'}`}
                onClick={() => {
                  setActiveIndex(i);
